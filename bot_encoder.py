...
TOKEN = '8081814013:AAFbLE2JP_yyA5B8foJzYRWANrVxEaHGw50'

# Пробуем разные подходы подключения
async def create_bot_session():
    """Создает сессию бота с разными вариантами подключения"""
    
    # Вариант 1: Без прокси (просто пробуем подключиться)
    try:
        print("🔄 Пробуем подключиться без прокси...")
        session = AiohttpSession()
        bot = Bot(token=TOKEN, session=session)
        await bot.get_me()  # Проверяем подключение
        print("✅ Успешное подключение без прокси!")
        return bot
    except Exception as e:
        print(f"❌ Без прокси не работает: {e}")
    
    # Вариант 2: Пробуем разные бесплатные прокси
    proxies = [
        'http://51.158.68.68:8811',
        'http://190.61.88.147:8080', 
        'http://45.224.148.117:999',
        'http://186.96.102.42:999',
    ...
    for proxy in proxies:
        try:
            ...
            bot = Bot(token=TOKEN, session=session)
            await bot.get_me()
            print(f"✅ Успех с прокси: {proxy}")
            return bot
        except Exception as e:
            print(f"❌ Прокси {proxy} не работает: {e}")
            continue
    
    # Вариант 3: Создаем бота без проверки (в надежде что заработает)
    print("⚠️ Создаем бота без проверки подключения...")
    session = AiohttpSession()
...
async def main():
    ...
    while retry_count < max_retries:
        try:
            ...
            wait_time = 10 * retry_count  # Увеличиваем время ожидания
